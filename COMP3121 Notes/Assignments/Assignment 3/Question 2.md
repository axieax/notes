...
function min_moves(grid, memo, target_x, target_y):
	...
	memo[(target_x, target_y)] <- (min_moves_current, previous_square);
	return min_moves_current;

// find the minimum of such moves from (1,R) to (C,1)
path_cost <- min_moves(grid, memo, C, 1);

// backtrack to find path
path <- empty list;
curr <- (C,1);
while curr != null:
	previous_square <- memo[curr]['previous'];
