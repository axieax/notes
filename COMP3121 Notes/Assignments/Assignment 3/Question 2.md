...
For this problem it can be observed that the minimum number of moves from lower elevation to higher elevation from square $(1,R)$​ to an arbitrary square $(x,y)$​ is given by the minimum of the costs for the two ways of getting to square $(x,y)$:
...
We can define a recursive algorithm for finding the minimum number of such moves as a subproblem. Here, I am also assuming that the number of moves from the question is referring to the total number of times along the path where there is an increase in elevation between squares, rather than the total number of elevation increases along the path. As mentioned above, the algorithm for calculating the minimum of such moves to $(x,y)$​ would involve finding the minimum of such moves to the square immediately above and to the left of $(x,y)$​ if they exist. If one of them does not exist (i.e. square $(x,y)$​ is located amongst the top or left edge of the grid), set them to be $\infty$​ since we cannot consider a path from that direction anyways (so that the minimum function considering both directions will avoid this direction). For left or above squares that do exist, we will need to recursively find the minimum number of such moves from square $(1,R)$​ to that square as a subproblem. If we were to get to square $(x,y)$​ from that previous left or above square, the minimum number of such moves from $(1,R)$​ to $(x,y)$​ would involve an additional cost if moving from that square to $(x,y)$​ means moving from lower elevation to higher elevation. This additional cost will need to be factored into whether it is more optimal to reach square $(x,y)$​​ from the square on the left, or from the square above, which can be solved by taking the minimum of such costs. For each of these comparisons, we will also need to store the more optimal previous square chosen into a predecessor matrix (whether the left or above square which would incur a smaller total cost). The computed minimum value can also be cached using memoization to improve the efficiency of the algorithm. Assuming paths begin on square $(1,R)$​, the base case is the number of such moves from square $(1,R)$​ to square $(1,R)$​​​​​​, which would be zero since there is no elevation difference. Notably, this is also the only square with no squares to its left or above it.
...
function min_moves(grid, pred, target_x, target_y):
	...
	if target_y + 1 <= R:
		...
		min_above_moves <- min_moves(grid, pred, target_x, target_y + 1);
	...
	if target_x - 1 >= 1:
		...
		min_left_moves <- min_moves(grid, pred, target_x - 1, target_y);
...
path_cost <- min_moves(grid, pred, C, 1);
